...
int      PartFinder (long *p, MrBFlt bl, MrBFlt theta, MrBFlt mu, int *partID);
...
void     SortIndParts (int *item, MrBFlt *assoc1, MrBFlt *assoc2, MrBFlt *assoc3, int count, int descendingOrder); /*prior for joint gene tree*/
void 	   SortIndParts2 (int *item, MrBFlt *assoc1, MrBFlt *assoc2, MrBFlt *assoc3, int left, int right, int descendingOrder);/*prior for joint gene tree*/
...
int		foundPound,foundpercent; /*prior for joint gene tree*/
MrBFlt	*treePartThetas,*aThetas,*sThetas,*treePartMus,*aMus,*sMus;/*prior for joint gene tree*/
...

{
	...
	if(jointGenePr == 1)
		{
		treePartThetas = NULL;/*prior for joint gene tree*/
		treePartMus = NULL;
		}
	...
	if(jointGenePr == 1)/*prior for joint gene tree*/
		{
		treePartThetas = (MrBFlt *)SafeMalloc((size_t) (2 * n * sizeof(MrBFlt)));
		if (!treePartThetas)
			{
			MrBayesPrint ("%s   Problem allocating treePartThetas (%d)\n", spacer, 2 * n * sizeof(MrBFlt));
			goto errorExit;
			}
		treePartMus = (MrBFlt *)SafeMalloc((size_t) (2 * n * sizeof(MrBFlt)));
		if (!treePartMus)
			{
			MrBayesPrint ("%s   Problem allocating treePartMus (%d)\n", spacer, 2 * n * sizeof(MrBFlt));
			goto errorExit;
			}
		}
	...
	if (sumtBrlensDef == YES)
		...
		aMus = NULL;
		aMus = (MrBFlt *)SafeMalloc((size_t) (MAX_PARTITIONS * sizeof(MrBFlt)));
		if (!aMus)
			{
			MrBayesPrint ("%s   Problem allocating aMus (%d)\n", spacer, MAX_PARTITIONS * sizeof(MrBFlt));
			goto errorExit;
			}
		aThetas = NULL;
		aThetas = (MrBFlt *)SafeMalloc((size_t) (MAX_PARTITIONS * sizeof(MrBFlt)));
		if (!aThetas)
			{
			MrBayesPrint ("%s   Problem allocating aThetas (%d)\n", spacer, MAX_PARTITIONS * sizeof(MrBFlt));
			goto errorExit;
			}
		...
		/*prior for joint gene tree*/
		sThetas = NULL;
		sThetas = (MrBFlt *)SafeMalloc((size_t) (MAX_PARTITIONS * sizeof(MrBFlt)));
		if (!sThetas)
			{
			MrBayesPrint ("%s   Problem allocating sThetas (%d)\n", spacer, MAX_PARTITIONS * sizeof(MrBFlt));
			goto errorExit;
			}
		sMus = NULL;
		sMus = (MrBFlt *)SafeMalloc((size_t) (MAX_PARTITIONS * sizeof(MrBFlt)));
		if (!sMus)
			{
			MrBayesPrint ("%s   Problem allocating sMus (%d)\n", spacer, MAX_PARTITIONS * sizeof(MrBFlt));
			goto errorExit;
			}
	...
	for (i=0; i<2*numTaxa; i++)
		...
		sumtNodes[i].index = 0;
		sumtNodes[i].theta = 0.0; /*prior for joint gene tree*/
		sumtNodes[i].mu = 0.0;
		}
	sumtRoot = NULL;
	...
	if (sumtBrlensDef == YES)
		{
		for (i=0; i<MAX_PARTITIONS; i++)
			aBrlens[i] = sBrlens[i] = aThetas[i] = sThetas[i] = aMus[i] = sMus[i] = 0.0;
		if (sumtParams.numRuns > 1)
			{
			for (i=0; i<MAX_PARTITIONS; i++)
...
#	endif
	...
	for (i=0; i<numTreePartsFound; i++)
		...
		/*prior for joint gene tree*/
		if(nBits == numTaxa && jointGenePr == 1)
			{
			conRoot->length = aBrlens[i];
			conRoot->theta = aThetas[i];
			conRoot->mu = aMus[i];
			continue;
			}
		...
		if (nBits > 1)
			...
			if (sumtBrlensDef == YES)
				{
				q->length = aBrlens[i];
				q->theta = aThetas[i]; /*prior for joint gene tree*/
				q->mu = aMus[i];
				}
			else
				{
				q->length = 0.0;
				q->theta = 0.0; /*prior for joint gene tree*/
				q->mu = 0.0;
				}
			q->support = freq * 100;
			q->x = nBits;
	...
	if(jointGenePr == 1)
		{
		q = (PolyNode *)SafeMalloc((size_t) (2 * numTaxa * sizeof(PolyNode)));
		if (!q)
			{
			MrBayesPrint ("%s   Could not allocate q\n", spacer);
			goto errorExit;
			}
		for(i=0; i<nextConNode; i++)
			q[i].length = conNodes[i].length;
		for(i=0; i<nextConNode; i++)
			{
			if(i == conRoot->index)
				conNodes[i].length = 0.0;
			else
				conNodes[i].length = q[conNodes[i].anc->index].length - conNodes[i].length;
			}
		free (q);
		}	
	...
	if(jointGenePr == 0)
		...
		{
		MrBayesPrintf (fpCon, "begin trees;\n");
		MrBayesPrintf (fpCon, "   [Note: This species tree contains information on the topology, \n");
		MrBayesPrintf (fpCon, "          branch lengths, populations sizes (if present), and the probability\n");
		MrBayesPrintf (fpCon, "          of the partition indicated by the branch.]\n");
		}
	...
	if (sumtBrlensDef == YES)
		...
		if(jointGenePr == 0)
			...
			MrBayesPrintf (fpCon, "\n");
			MrBayesPrintf (fpCon, "   [Note: This tree contains information only on the topology\n");
			MrBayesPrintf (fpCon, "          branch lengths and population sizes (mean of the posterior probability density).]\n");
			}
		if (!strcmp(sumtParams.sumtConType, "Halfcompat"))
			MrBayesPrintf (fpCon, "   tree con_50_majrule = ");
		else
...

{
	...
	MrBFlt		vartheta=0.0,varmu=0.0; /*prior for joint gene tree*/
	long		*x;
...
#	endif
	...
	for (treeNo = 0; treeNo < sumtParams.numTrees; treeNo++)
		...
		if (sumtBrlensDef == YES)
			{
			MrBayesPrint ("   Mean(v)   Var(v)   Mean(The)  Var(The)  Mean(Mu)  var(Mu)"); /*prior for joint gene tree*/
			tableWidth += 60;
			if (sumtParams.numRuns > 1)
				{
				MrBayesPrint ("   PSRF");
		...
		for (i=0; i<numTreePartsToPrint; i++)
			...
			if (sumtBrlensDef == YES)
				{
				if (numFoundOfThisPart[i] == 1) /*prior for joint gene tree*/
					{
					var = 0.0;
					vartheta = 0.0;
					varmu = 0.0;
					...
					vartheta = sThetas[i] / (numFoundOfThisPart[i]-1);
					varmu = sMus[i]/(numFoundOfThisPart[i]-1);
					}
				}
			if (sumtParams.numRuns > 1)
				{
			...
			if (sumtBrlensDef == YES)
				{
				MrBayesPrint ("  %1.6lf  %1.6lf  %1.6lf  %1.6lf  %1.6lf  %1.6lf", aBrlens[i], var, aThetas[i], vartheta,aMus[i],varmu); /*prior for joint gene tree*/
				if (sumtParams.numRuns > 1)
					{
					if (varW <= 0.0)
						MrBayesPrint ("  > 2.0");
		...
		if (sumtBrlensDef == YES)
			{
			MrBayesPrintf (fpParts, "\t\tBRLEN\t\tSTD\t\tTheta\t\tSTD\t\tMu\t\tSTD"); /*prior for joint gene tree*/
			if (sumtParams.numRuns > 1)
		...
		for (i=0; i<numTreePartsToPrint; i++)
			...
			if (sumtBrlensDef == YES)
				{
				MrBayesPrintf (fpParts, "\t%1.6lf\t%1.6lf\t%1.6lf\t%1.6lf\t%1.6lf\t%1.6lf", aBrlens[i], sqrt(var), aThetas[i], sqrt(vartheta),aMus[i], sqrt(varmu));
				if (sumtParams.numRuns > 1)
					{
					if (varW <= 0.0)
...

{
	...
	if (expecting == Expecting(PARAMETER))
	...
	else if (expecting == Expecting(POUNDSIGN)) /*prior for joint gene tree*/
		{
		foundPound = YES;
		expecting  = Expecting(NUMBER);
		}
	else if (expecting == Expecting(PERCENT)) /*prior for joint gene tree*/
		{
		foundpercent = YES;
		expecting  = Expecting(NUMBER);
		}
		...
		else if (foundPound == YES && jointGenePr == 1) /*prior for joint gene tree*/
			{
			/* THETA */
			sscanf (tkn, "%lf", &tempD);
			if (pSumtPtr->marked == NO)
				pSumtPtr->theta = tempD;
			else
				{
				if (pSumtPtr->left != NULL)
					pSumtPtr->left->theta = tempD;
				else
					{
					MrBayesPrint ("%s   Cannot assign population size theta to left node\n", spacer);
					goto errorExit;
					}
				}
			foundPound = NO;
		...
		else if (foundpercent == YES && jointGenePr == 1) /*prior for joint gene tree*/
			{
			/* MU */
			sscanf (tkn, "%lf", &tempD);
			if (pSumtPtr->marked == NO)
				...
				{
				if (pSumtPtr->left != NULL)
					pSumtPtr->left->mu = tempD;
				else
					{
					MrBayesPrint ("%s   Cannot assign mutation rate to left node\n", spacer);
					goto errorExit;
					}
				}
			foundpercent = NO;
...

{
	...
	if(jointGenePr == 1)
		...
		for (i=0; i<2*numTaxa-1; i++)
			...
			bl = 0.0;
			p = &sumtNodes[i];
			while(p->index >= numTaxa)
				{
				p = p->left;
				bl += p->length;
				}
			sumtNodes[i].length = bl;
			}
		}
	...
	for (i=0; i<nNodes; i++)
		...
		if (p->anc != NULL)
			...
			if (p->anc->anc != NULL)
				{
				if (PartFinder (&treeBits[(p->index) * taxonLongsNeeded], bl, p->theta, p->mu, &partNum) == ERROR)
					goto errorExit;
				treePartNums[j] = partNum;
				treePartLengths[j] = bl;
				if(jointGenePr == 1) /*prior for joint gene tree*/
					{
					treePartThetas[j] = p->theta;
					treePartMus[j] = p->mu;
					}
				j++;
				}
			...
			else if (p->anc->anc == NULL && jointGenePr == 1) /*prior for joint gene tree*/
				{
				if (PartFinder (&treeBits[(p->index) * taxonLongsNeeded], bl, p->theta, p->mu, &partNum) == ERROR)
					goto errorExit;
				treePartNums[j] = partNum;
	...
	numTreeParts = j;
	SortIndParts (&treePartNums[0], &treePartLengths[0], &treePartThetas[0], &treePartMus[0], numTreeParts, YES);/*prior for joint gene tree*/
...
#	if 0
	...
	for (i=0; i<numTreeParts; i++)
		MrBayesPrint ("      %3d %lf %lf %lf\n", treePartNums[i], treePartLengths[i], treePartThetas[i], treePartMus[i]);/*prior for joint gene tree*/
#	endif
...

{
	...
	if (memAllocs[ALLOC_TREEBITS] == YES)
		...
		free (treePartThetas); /*prior for joint gene tree*/
		free (treePartMus);
	...
	if (memAllocs[ALLOC_ABRLENS] == YES)
		{
		free (aBrlens);
		free (aThetas); /*prior for joint gene tree*/
		free (aMus);
	...
	if (memAllocs[ALLOC_SBRLENS] == YES)
		{
		free (sBrlens);
		free (sThetas); /*prior for joint gene tree*/
		free (sMus);
...

{
	...
	if (bl >= 0.0)
		...
		if (numFoundOfThisPart[whichPart] == 1)
			...
			aThetas[whichPart] = theta; /*prior for joint gene tree*/
			sThetas[whichPart] = 0.0;   /*prior for joint gene tree*/
			aMus[whichPart] = mu; /*prior for joint gene tree*/
			sMus[whichPart] = 0.0;   /*prior for joint gene tree*/
			}
		else
			{
			f = aBrlens[whichPart];
			...
			f = aThetas[whichPart];
			aThetas[whichPart] += (theta - aThetas[whichPart]) / (MrBFlt) (numFoundOfThisPart[whichPart]); /*prior for joint gene tree*/
			sThetas[whichPart] += (theta - aThetas[whichPart]) * (theta - f); /*prior for joint gene tree*/

			f = aMus[whichPart];
			aMus[whichPart] += (mu - aMus[whichPart]) / (MrBFlt) (numFoundOfThisPart[whichPart]); /*prior for joint gene tree*/
			sMus[whichPart] += (mu - aMus[whichPart]) * (mu - f); /*prior for joint gene tree*/
			}
		if (sumtParams.numRuns > 1)
			{
			if (numFoundOfThisPart[whichPart] == 1)
...

{
	...
	if (sumtBrlensDef == YES)
		...
		/*prior for joint gene tree*/
		aThetas = (MrBFlt *)realloc(aThetas, (size_t) (numPartsAllocated * sizeof(MrBFlt)));
		if (!aThetas)
			{
			MrBayesPrint ("%s   Problem reallocating aThetas (%d)\n", spacer, numPartsAllocated * sizeof(MrBFlt));
			goto errorExit;
			}
		aMus = (MrBFlt *)realloc(aMus, (size_t) (numPartsAllocated * sizeof(MrBFlt)));
		if (!aMus)
			{
			MrBayesPrint ("%s   Problem reallocating aMus (%d)\n", spacer, numPartsAllocated * sizeof(MrBFlt));
			goto errorExit;
			}
		...
		/*prior for joint gene tree*/
		sThetas = (MrBFlt *)realloc(sThetas, (size_t) (numPartsAllocated * sizeof(MrBFlt)));
		if (!sThetas)
			{
			MrBayesPrint ("%s   Problem reallocating sThetas (%d)\n", spacer, numPartsAllocated * sizeof(MrBFlt));
			goto errorExit;
			}
		sMus = (MrBFlt *)realloc(sMus, (size_t) (numPartsAllocated * sizeof(MrBFlt)));
		if (!sMus)
			{
			MrBayesPrint ("%s   Problem reallocating sMus (%d)\n", spacer, numPartsAllocated * sizeof(MrBFlt));
			goto errorExit;
			}
		}
...
/*prior for joint gene tree*/
void SortIndParts (int *item, MrBFlt *assoc1, MrBFlt *assoc2, MrBFlt *assoc3, int count, int descendingOrder)

{

	SortIndParts2 (item, assoc1, assoc2, assoc3, 0, count-1, descendingOrder);
...
/*prior for joint gene tree*/
void SortIndParts2 (int *item, MrBFlt *assoc1, MrBFlt *assoc2, MrBFlt *assoc3, int left, int right, int descendingOrder)

{
	...
	if (descendingOrder == YES)
		...
		if (left < j)
			SortIndParts2 (item, assoc1, assoc2, assoc3, left, j, descendingOrder);
		if (i < right)
			SortIndParts2 (item, assoc1, assoc2, assoc3, i, right, descendingOrder);
		}
	else
		{
		i = left;
...

{
	...
	if (p->left == NULL)
		{
		if (sumtBrlensDef == YES) /*prior for joint gene tree*/
			fprintf (fp, "%s:%lf", p->label, p->length);
		else
			fprintf (fp, "%s", p->label);
		if (jointGenePr == 1 && p->theta > 0.0) /*prior for joint gene tree*/
			fprintf (fp, "#%lf", p->theta);
		if (jointGenePr == 1 && p->mu > 0.0) /*prior for joint gene tree*/
			fprintf (fp, "%%%lf", p->mu);
		}
	...
	if (p->sib == NULL && p->anc != NULL)
		...
		if (p->anc->anc != NULL)
			...
			if(jointGenePr == 0)
				...
				if (sumtBrlensDef == YES && showSupport == NO)
					{
					fprintf (fp, "):%lf#%lf", p->anc->length,p->anc->theta); 
					if(p->anc->mu>0.0)
						fprintf (fp, "%%%lf", p->anc->mu); 
					}
				else if (sumtBrlensDef == NO && showSupport == YES)
					fprintf (fp, ")%1.2lf", p->anc->support/100.0); 
				else if (sumtBrlensDef == YES && showSupport == YES)
					{
					fprintf (fp, ")%1.2lf:%lf#%lf", p->anc->support/100.0, p->anc->length,p->anc->theta);
					if(p->anc->mu>0.0)
						fprintf (fp, "%%%lf", p->anc->mu); 
					}
				else
			...
			if(jointGenePr == 1)
				{
				if(p->anc->mu>0.0)
					fprintf (fp, ")[#%lf%lf]",p->anc->theta,p->anc->mu);
				else
					fprintf (fp, ")[#%lf]",p->anc->theta);
				}
			else
				fprintf (fp, ")");
			}
