...
void FindDescendantTaxa(SPTree *tree, int inode, int *taxa, int *index)
{
	...
	else
	{
		...
		FindDescendantTaxa(tree, son0, taxa, index);
		son1 = tree->nodes[inode].sons[1];
		FindDescendantTaxa(tree, son1, taxa, index);
	}
}
...
{ 
   	...
   	Distance *minimumdistance, onetreeConstraint[NSPECIES];


   	minimumdistance = (Distance*)malloc(nGene*speciestree->nTaxa*(speciestree->nTaxa - 1)/2*sizeof(Distance)); 
   	if(!minimumdistance)
   	{
	   	printf(" Allocation problem for minimumdistance\n");
	   	return(ERROR);
   	}

	nconstraints = ClockTreeNodeDist(gtree, nGene, minimumdistance);
	SPTreeConstraint(minimumdistance, onetreeConstraint, nconstraints, speciestree->nSpecies);

	/*for debug*/
	/*if(CheckConstraint(gtree, nGene, onetreeConstraint, speciestree->nSpecies-1) == ERROR)
	{
		printf("Constraint is wrong!\n");
		return ERROR;
	}*/

	/*bifurcating species tree*/
			...
			if(onetreeConstraint[i].dist == onetreeConstraint[j].dist)
				{
					...
					if(onetreeConstraint[j].dist < 0.0)
						onetreeConstraint[j].dist = 0.0;
				}
		}
...
int	MoveNode(SPTree *speciestree)
{
	int i, node,k,j;
	double oldC, newC;
	double position[2],window=0.0001;
	int treeroot;

	/*change the tree constraint*/
	i = (int)(rndu()*(speciestree->nSpecies - 1));
      	oldC = speciestree->treeConstraint[i].dist;
	newC = oldC - rndu()*window;
  	...
  	if(newC < 1e-10) 
       ...
       /*newC = rndu() * oldC;*/
       speciestree->treeConstraint[i].dist = newC;

	/*construct a new tree according to the new constraint*/
	FOR(i,2)      
		{   node = speciestree->treeConstraint[0].nodes[i];
			speciestree->nodes[node].father=speciestree->nSpecies;
			speciestree->nodes[node].nson=0;
			FOR(j,2) speciestree->nodes[node].sons[j]=-2;
			speciestree->nodes[node].brlens = speciestree->treeConstraint[0].dist/2;
			speciestree->nodes[node].age = 0.0;
		}

  
	treeroot=speciestree->nSpecies;
	speciestree->nodes[speciestree->nSpecies].nson=2;
	speciestree->nodes[speciestree->nSpecies].sons[0]=speciestree->treeConstraint[0].nodes[0];
	speciestree->nodes[speciestree->nSpecies].sons[1]=speciestree->treeConstraint[0].nodes[1];
	speciestree->nodes[speciestree->nSpecies].brlens=0.0;
	speciestree->nodes[speciestree->nSpecies].age = speciestree->nodes[node].brlens;
 
	for(i=2;i<speciestree->nSpecies;i++)
	{   
		node = speciestree->treeConstraint[i-1].nodes[1];
		speciestree->nodes[node].father=speciestree->nSpecies+i-1;
		speciestree->nodes[node].nson=0;
		speciestree->nodes[node].sons[0]=-2;
		speciestree->nodes[node].sons[1]=-2;
		speciestree->nodes[node].brlens=speciestree->treeConstraint[i-1].dist/2;
		speciestree->nodes[node].age = 0.0;
        
		/*find the position of the nodes and the distance*/
		if(FindaPosition(speciestree->treeConstraint[i-1].nodes[0],treeroot,speciestree->nodes[node].brlens,position,speciestree)==ERROR)
		{
		  return(ERROR);  
		}
   	  
		speciestree->nodes[speciestree->nSpecies+i-1].father=speciestree->nodes[(int)position[0]].father;
		speciestree->nodes[speciestree->nSpecies+i-1].nson=2;
		speciestree->nodes[speciestree->nSpecies+i-1].sons[0]=node;
		speciestree->nodes[speciestree->nSpecies+i-1].sons[1]=(int)position[0];
        
		if(position[0]==treeroot) 
		{
			treeroot=speciestree->nSpecies+i-1;
			speciestree->nodes[speciestree->nSpecies+i-1].brlens=0.0;
            speciestree->nodes[speciestree->nSpecies+i-1].age = speciestree->nodes[node].brlens;

		}
		else
		{  
			speciestree->nodes[speciestree->nSpecies+i-1].brlens = speciestree->nodes[(int)position[0]].brlens - position[1];  
            speciestree->nodes[speciestree->nSpecies+i-1].age = speciestree->nodes[node].brlens;

			FOR(k,2) 
			{
				if(speciestree->nodes[speciestree->nodes[(int)position[0]].father].sons[k]==position[0]) 
				{
					speciestree->nodes[speciestree->nodes[(int)position[0]].father].sons[k]=speciestree->nSpecies+i-1;
					break;
				}
			}
		}

	  speciestree->nodes[(int)position[0]].father=speciestree->nSpecies+i-1;
	  speciestree->nodes[(int)position[0]].brlens=position[1];        
	}  
	speciestree->root=treeroot;
	speciestree->nodes[treeroot].father=-1;
